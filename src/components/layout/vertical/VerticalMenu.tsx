...
import { Currency, LayoutDashboard, List, SearchCheck, ShieldCheck } from 'lucide-react'
...
const VerticalMenu = ({ scrollMenu }: { scrollMenu: (container: any, isPerfectScrollbar: boolean) => void }) => {
  ...
  return (
    ...
    >
      ...
      >
        <MenuItem icon={<LayoutDashboard className='w-4 h-4' />} href='/'>
          Analytics
        </MenuItem>

        <MenuSection label='Your Budgets & Expences'>
          <SubMenu
            label='Tracker'
          ...
          >
            {adminRoutes.map(route => (
              <MenuItem key={route.href} icon={<route.Icon className='w-4 h-4' />} href={route.href}>
                {route.label}
              </MenuItem>
            ))}
